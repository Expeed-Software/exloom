...
`exloom:executing-handoff-plans` and `exloom:orchestrating-execution` — run it
...
| Parallel implementers that may touch overlapping files | see `exloom:orchestrating-execution` — a worktree per implementer |
...
**fan-out** — `exloom:orchestrating-execution` gives each parallel implementer its
...
- **Before:** `exloom:executing-handoff-plans` and `exloom:orchestrating-execution`
  both run this first.
